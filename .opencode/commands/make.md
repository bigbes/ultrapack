...
Invoke `udesign`. It populates `## Design`, `### Invariants` (IV), `### Principles` (PC), `### Assumptions` (AS), `### Unknowns` (UK), and records `TDD: yes / no (reason)`. Status → `planning`.
...
Invoke `uplan`. It populates `## Plan`. Status → `executing`.
...
Status → `reviewing`. Invoke `ureview`. It dispatches `ureviewer`, processes findings, fills `## Conclusion`. Status → `done`.
