...
- **Prefer additive over subtractive edits.** Rename before deleting. Comment-out before removing. Add a new file before replacing the old one in place. The reviewer (`@reviewer`) still catches unused cruft; better that than a deleted file the user wanted.
