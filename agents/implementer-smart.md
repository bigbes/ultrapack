---
description: Implement one phase of an approved plan on the smart tier (Kimi K2.6) — for nuanced, balanced phases where output quality matters more than throughput. Dispatched per-phase from uexecute when the plan declares `Tier: smart`. Same contract as @implementer.
