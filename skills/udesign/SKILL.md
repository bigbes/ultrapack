...
8. Decide TDD — yes or no, with reason. Use `test-driven-development`'s applicability rule.
...
11. Wait for user approval before invoking `uplan`.
</required>
...
Invoke the applicability rule from `test-driven-development`. Record in Design:
...
- Omit empty subsections. `### Invariants`, `### Principles`, `### Assumptions`, `### Unknowns` are pre-seeded by the `/make` template. Delete any that end up with no entries — never leave a placeholder like `<empty>`, "none", or "n/a". See `_brevity.md` principle 1.
...
User has approved the Design section (interactive) or the Design has been written and self-reviewed (hands-off) → invoke `uplan`. Do not write code. Do not invoke any other skill.
