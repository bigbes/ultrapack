...
Implement the approved `## Plan` from `docs/tasks/<slug>.md`. You are the dispatcher — each phase is handed to a fresh `@implementer` subagent. After each phase returns, you run the plan-diff check and consistency pass before moving on. The goal is a working change that honors Design and Plan.
...
4. Build the checklist — one todo per plan phase (or per task if phases are coarse). Use an explicit checklist.
</required>
...
<required>
1. Mark the phase(s) `in_progress` in an explicit checklist.
...
3. On implementer return, handle status:
   ...
   - `BLOCKED` → diagnose. If the plan is wrong, invoke `uplan`. If context-only, re-dispatch. Do not retry identically.
...
Used when the Plan declares `### Interface graph` (written by `uplan`). Waves are derived by topo-sort — never hand-declared (PC5).
...
3. **Boundary check (IF5):** run `git show <sha> --name-only`. Every changed path must be a member of the phase's declared `@` set. On trespass: halt the wave, log under `### Deferred (needs user input)` with the trespassed path and phase, then either re-dispatch with tightened scope or escalate to `uplan`.
...
5. On mismatch: log under `## Conclusion → ### Deviations from plan` with `IF<n>: <mismatch description>`. Decide whether to re-dispatch the offending phase (implementation error), invoke `uplan` (plan declared the wrong signature), or continue with a recorded deviation.
</required>

## TDD

If Design recorded `TDD: yes`, invoke `test-driven-development` for each unit under test:
...
If `TDD: no`, skip the test-first loop; verification happens in `uverify`.

## When to dispatch `@explorer`

- The implementer reported `NEEDS_CONTEXT` and a code map would unblock them
- You need a call graph or execution path beyond what a quick text search gives
- You need a ranked list of essential files for a feature

Dispatch with tight scope and pass the working directory explicitly. Don't over-use — inline search/read beats a subagent for one-shot lookups.
...
If the question is purely about the current codebase, prefer `@explorer`. If it's a single doc lookup, inline Context7 is enough.
...
4. If the deviation is structural enough that later phases in the plan no longer apply — stop executing. Invoke `uplan` with enough context (what was done, what no longer applies, what new reality is). Let the planner skill update the plan before resuming.
</required>
...
All phases done and committed → invoke `uverify`. Do not skip to review. Do not finish the branch yourself.
