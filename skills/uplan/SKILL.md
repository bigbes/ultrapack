...
Convert the approved Design into a lean, concrete implementation plan. Output fills the `## Plan` section of `docs/tasks/<slug>.md`. The plan is the contract that `uexecute` will follow.
...
11. Present the plan to the user. In interactive mode, wait for approval, then invoke `uexecute`. In hands-off mode (task-file `**Mode:** hands-off`), auto-invoke `uexecute` after presenting, and log `- uplan: plan auto-approved (hands-off)` to `## Conclusion → ### Hands-off decisions`.
</required>
...
- `@ <paths>` marks the filesystem boundary for each phase. Paths declared by phases in the same wave must be disjoint; `uexecute` uses this to detect boundary violations after each commit.
...
<required>
Read `.opencode/references/principles.md` (or `~/.config/opencode/references/principles.md` for global installs) before sketching phases. Every phase must be consistent with GPC1–GPC8; where a phase deviates, the bullet names the GPC and the reason in one line (e.g. "breaks GPC3 — single call site, split would obscure the flow").
</required>
...
See `handsoff` for the full contract. Stage-specific delta: skip the approval wait — present the plan highlights, log `- uplan: plan auto-approved` to `### Hands-off decisions`, invoke `uexecute` directly. Self-review, scope-creep check, and backwards-compat restatement are unchanged. Ambiguities that would require a user call go to `### Deferred (needs user input)` and the plan stops — do not guess around them.

## Terminal state

Plan written, self-reviewed, scope-checked. Interactive: present highlights, wait for approval, invoke `uexecute`. Hands-off: present highlights, log auto-approval, invoke `uexecute`.
